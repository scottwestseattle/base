@extends('layouts.app')
@section('title', trans_choice('view.Word', 2))
@section('menu-submenu')@component('words.menu-submenu')@endcomponent @endsection
@section('content')
<div>
	<h1>{{trans_choice('view.Word', 2)}} ({{count($records)}})</h1>
	...
	<table class="table">
		...
		@foreach($records as $record)
			<tr>
				...
				<td class="icon"><a href='/words/publishupdate/{{$record->id}}'>@component('components.icon', ['svg' => 'lightning'])@endcomponent</a></td>
				<td class="index-button">@component('components.button-release-status', ['record' => $record, 'views' => 'words', 'class' => 'btn-xxs'])@endcomponent</td>
				...
				<!-- td class="icon"><a href='/words/confirmdelete/{{$record->id}}'>@component('components.icon', ['svg' => 'trash-fill'])@endcomponent</a></td -->
			</tr>
		@endforeach
		</tbody>
	</table>
