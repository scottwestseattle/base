@extends('layouts.app')
@section('title', trans_choice('base.Comment', 2))
@section('menu-submenu')@component('comments.menu-submenu')@endcomponent @endsection
@section('content')
<div>
	<h1>{{trans_choice('base.Comment', 2)}} ({{count($records)}})</h1>
	...
	<table class="table">
		...
		@foreach($records as $record)
			<tr>
				...
				<td class="icon"><a href='/comments/publish/{{$record->id}}'>@component('components.icon', ['svg' => 'lightning'])@endcomponent</a></td>
				<td>{{$record->id}}</td>
				<td><a href="/comments/view/{{ $record->id }}">{{$record->title}}</a></td>
				<td>{{$record->description}}</td>
				<td>{{$record->created_at}}</td>
				<td class="icon"><a href='/comments/confirmdelete/{{$record->id}}'>@component('components.icon-delete')@endcomponent</span></a></td>
			</tr>
		@endforeach
		</tbody>
	</table>
