@extends('layouts.app')
@section('title', __('proj.Deleted Exercises'))
@section('menu-submenu')@component('gen.exercises.menu-submenu')@endcomponent @endsection
@section('content')
<div>
	<h1>{{__('proj.Deleted Exercises')}} ({{count($records)}})</h1>
	...
	<table class="table">
		...
		@foreach($records as $record)
			<tr>
				<td class="index-button"><a href='/exercises/undelete/{{$record->id}}'>{{__('ui.Undelete')}}</a></td>
				<td>{{$record->title}}</td>
