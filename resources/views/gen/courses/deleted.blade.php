@extends('layouts.app')
@section('title', __('proj.Deleted Courses'))
@section('menu-submenu')@component('gen.courses.menu-submenu')@endcomponent @endsection
@section('content')
<div>
	<h1>{{__('proj.Deleted Courses')}} ({{count($records)}})</h1>
	...
	<table class="table">
		...
		@foreach($records as $record)
			<tr>
				<td class="index-button"><a href='/courses/undelete/{{$record->id}}'>{{__('ui.Undelete')}}</a></td>
				<td>{{$record->title}}</td>
