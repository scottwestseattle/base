@extends('layouts.app')
@section('title', __('proj.Deleted Contacts'))
@section('menu-submenu')@component('gen.contacts.menu-submenu')@endcomponent @endsection
@section('content')
<div>
	<h1>{{__('proj.Deleted Contacts')}} ({{count($records)}})</h1>
	...
	<table class="table">
		...
		@foreach($records as $record)
			<tr>
				<td class="index-button"><a href='/contacts/undelete/{{$record->id}}'>{{__('ui.Undelete')}}</a></td>
				<td>{{$record->title}}</td>
