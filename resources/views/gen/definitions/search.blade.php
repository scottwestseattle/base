...
@section('content')
	...
	<div class="mb-3">
		<form method="POST" action="/dictionary/create">
			<input type="text" id="title" name="title" value="{{$search}}" class="form-control" autocomplete="off" onfocus="$(this).select(); setFocus($(this));" onkeyup="searchDefinitions(event, '#title', '#searchResults', '/definitions/search-ajax/');" autofocus />
		</form>
	</div>
